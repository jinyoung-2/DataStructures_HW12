...
/* 버블 정렬 
	-> 두 인접원소를 검사하여 정렬하는 방법으로, 최대값을 비정렬된 부분 중 제일 뒤로 보내는 것을 반복한다(뒤에서부터 정렬)*/
int bubbleSort(int *a)
{
	int i, j, t;
	...
	for(i = 0; i < MAX_ARRAY_SIZE; i++)
	{
		...
		{
			//인접한 두 원소 중 앞의 원소가 클 때 -> 교환
			if (a[j-1] > a[j])	//ERROR: j=0일 때, a[j-1]=a[-1]로 해당 배열값은 쓰레기값이 저장되어 있어 쓰레기값이 정렬과정에 포함되어 잘못된 정렬이 발생한다. 
			{
